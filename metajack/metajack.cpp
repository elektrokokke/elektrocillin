...
{
    return MetaJackContext::getInstance()->set_client_registration_callback(client, registration_callback, arg);
}
...
{
    return MetaJackContext::getInstance()->set_port_registration_callback(client, registration_callback, arg);
}

int meta_jack_set_port_connect_callback (jack_client_t *client, JackPortConnectCallback connect_callback, void *arg)
{
    return MetaJackContext::getInstance()->set_port_connect_callback(client, connect_callback, arg);
}

int meta_jack_set_port_rename_callback (jack_client_t *client, JackPortRenameCallback rename_callback, void *arg)
{
    return MetaJackContext::getInstance()->set_port_rename_callback(client, rename_callback, arg);
}

int meta_jack_set_graph_order_callback (jack_client_t *client, JackGraphOrderCallback graph_callback, void *arg)
{
    return MetaJackContext::getInstance()->set_graph_order_callback(client, graph_callback, arg);
}

int meta_jack_set_xrun_callback (jack_client_t *client, JackXRunCallback xrun_callback, void *arg)
{
    return MetaJackContext::getInstance()->set_xrun_callback(client, xrun_callback, arg);
}

int meta_jack_set_freewheel(jack_client_t *client, int onoff)
{
    return MetaJackContext::getInstance()->set_freewheel(client, onoff);
}

int meta_jack_set_buffer_size (jack_client_t *client, jack_nframes_t nframes)
{
    return MetaJackContext::getInstance()->set_buffer_size(client, nframes);
}

jack_nframes_t meta_jack_get_sample_rate (jack_client_t *client)
{
    return MetaJackContext::getInstance()->get_sample_rate(client);
}

jack_nframes_t meta_jack_get_buffer_size (jack_client_t *client)
{
    return MetaJackContext::getInstance()->get_buffer_size(client);
}

float meta_jack_cpu_load (jack_client_t *client)
{
    return MetaJackContext::getInstance()->cpu_load(client);
}
...
{
    return MetaJackContext::getInstance()->port_register(client, port_name, port_type, flags, buffer_size);
}

int meta_jack_port_unregister (jack_client_t *client, jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_unregister(client, port);
}

void * meta_jack_port_get_buffer (jack_port_t *port, jack_nframes_t nframes)
{
    return MetaJackContext::getInstance()->port_get_buffer(port, nframes);
}

const char * meta_jack_port_name (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_name(port);
}

const char * meta_jack_port_short_name (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_short_name(port);
}

int meta_jack_port_flags (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_flags(port);
}

const char * meta_jack_port_type (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_type(port);
}

int meta_jack_port_is_mine (const jack_client_t *client, const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_is_mine(client, port);
}

int meta_jack_port_connected (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_connected(port);
}

int meta_jack_port_connected_to (const jack_port_t *port, const char *port_name)
{
    return MetaJackContext::getInstance()->port_connected_to(port, port_name);
}

const char ** meta_jack_port_get_connections (const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_get_connections(port);
}

const char ** meta_jack_port_get_all_connections (const jack_client_t *client, const jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_get_all_connections(client, port);
}

jack_nframes_t meta_jack_port_get_latency (jack_port_t *port)
{
   return MetaJackContext::getInstance()->port_get_latency(port);
}

jack_nframes_t meta_jack_port_get_total_latency (jack_client_t *client, jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_get_total_latency(client, port);
}

void meta_jack_port_set_latency (jack_port_t *port, jack_nframes_t nframes)
{
    return MetaJackContext::getInstance()->port_set_latency(port, nframes);
}

int meta_jack_recompute_total_latency (jack_client_t *client, jack_port_t *port)
{
    return MetaJackContext::getInstance()->recompute_total_latency(client, port);
}

int meta_jack_recompute_total_latencies (jack_client_t *client)
{
    return MetaJackContext::getInstance()->recompute_total_latencies(client);
}

int meta_jack_port_set_name (jack_port_t *port, const char *port_name)
{
    return MetaJackContext::getInstance()->port_set_name(port, port_name);
}

int meta_jack_port_set_alias (jack_port_t *port, const char *alias)
{
    return MetaJackContext::getInstance()->port_set_alias(port, alias);
}

int meta_jack_port_unset_alias (jack_port_t *port, const char *alias)
{
    return MetaJackContext::getInstance()->port_unset_alias(port, alias);
}

int meta_jack_port_get_aliases (const jack_port_t *port, char* const aliases[])
{
    return MetaJackContext::getInstance()->port_get_aliases(port, aliases);
}

int meta_jack_port_request_monitor (jack_port_t *port, int onoff)
{
    return MetaJackContext::getInstance()->port_request_monitor(port, onoff);
}

int meta_jack_port_request_monitor_by_name (jack_client_t *client, const char *port_name, int onoff)
{
    return MetaJackContext::getInstance()->port_request_monitor_by_name(client, port_name, onoff);
}

int meta_jack_port_ensure_monitor (jack_port_t *port, int onoff)
{
    return MetaJackContext::getInstance()->port_ensure_monitor(port, onoff);
}

int meta_jack_port_monitoring_input (jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_monitoring_input(port);
}

int meta_jack_connect (jack_client_t *client, const char *source_port, const char *destination_port)
{
    return MetaJackContext::getInstance()->connect(client, source_port, destination_port);
}

int meta_jack_disconnect (jack_client_t *client, const char *source_port, const char *destination_port)
{
    return MetaJackContext::getInstance()->disconnect(client, source_port, destination_port);
}

int meta_jack_port_disconnect (jack_client_t *client, jack_port_t *port)
{
    return MetaJackContext::getInstance()->port_disconnect(client, port);
}

int meta_jack_port_name_size()
{
    return MetaJackContext::getInstance()->port_name_size();
}

int meta_jack_port_type_size()
{
    return MetaJackContext::getInstance()->port_type_size();
}
...
{
    MetaJackContext::getInstance()->set_error_function(func);
}

void meta_jack_set_info_function (void (*func)(const char *))
{
    MetaJackContext::getInstance()->set_info_function(func);
}
