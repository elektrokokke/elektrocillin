...
//    return interpolator.evaluate(phase);
    ...
    for (; t1 < tmax; ) {
        ...
        if (x2 != x1) {
            ...
            double b = interpolator.getY()[i] - interpolator.getX()[i] * a * M_PI * 2;
            double integral1 = (a * phase * M_PI * 2 + b) * si1 - a * getNormalizedFrequency() * 2 * cos(t1 * M_PI);
            double integral2 = (a * phase * M_PI * 2 + b) * si2 - a * getNormalizedFrequency() * 2 * cos(t2 * M_PI);
